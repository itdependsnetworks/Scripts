...
my (%nethash,%maskhash,$interface_check,@dns)=();
foreach my $filename (@filecheck){
        ...
        foreach my $line (@conf){
                if ($line =~ /^hostname (.+)$/){
                        ...
                        if (lc($hostname) ne lc($device)){
                                push @hostname, "Hostname Mismatch: Device $hostname - File $device\n"
                        }
                }
                if ($line =~ /^interface (.+?)\s/){
                        $start = 1;
		...
		if ($interface_check && $line =~/^!/ && $ip_address && $interface_hold){
			#$interface_hold =~ s/\./_/g;
			#$interface_hold =~ s/\/./_/g;
	                $interface_hold =~ s/\./_/g;
        	        $interface_hold =~ s/:/_/g;
                	$interface_hold =~ s/\//_/g;	
			...
			print "$ip_address $hostname-$interface_hold\n";
			push @dns, "$ip_address $hostname-$interface_hold"; 
			($subnet_mask,$masklen,$network,$sort,$ip_address) = ();
		}
